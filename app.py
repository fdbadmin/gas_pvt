...
try:
    from report_generator import generate_pvt_report
    _HAS_REPORT = True
except ImportError:
    _HAS_REPORT = False
...
def main():
    ...
    # Sidebar for navigation – grouped by category
    st.sidebar.title("📊 Analysis Type")

    st.sidebar.markdown("**Core PVT**")
    ...
    # Separator labels are not real pages
    page_map = {
    ...
    st.sidebar.info(
        ...
        "- **Z-factor**: Hall-Yarborough, DAK, Piper-McCain-Corredor\n"
        "- **Viscosity**: Lucas with Standing correction\n"
        "- **Critical Properties**: Sutton, Riazi-Daubert, PMC\n"
        "- **Water Properties**: McCain, Whitson-Brule\n"
        "- **Hydrate**: Katz, Hammerschmidt\n"
...
# ============================================================================
# NEW PAGES — Phase 1–4 Feature Expansion
# ============================================================================

# ---------- Z-factor Comparison (Step 2) ------------------------------------
...
def zfactor_comparison():
    ...
    with col2:
        if calc:
            ...
            # Pseudo-criticals — Sutton (for HY/DAK) and PMC
            ...
            # Filter NaN for plotting (DAK/PMC may return NaN outside valid range)
            import math as _m
...
# ---------- OGIP / P÷Z Material Balance (Step 3) ---------------------------
...
def ogip_material_balance():
    ...
    # Use uploaded data if available, otherwise use manual entry
...
# ---------- Wellbore Pressure Traverse (Step 4) -----------------------------
...
def wellbore_traverse_page():
    ...
    with col2:
        if calc:
            ...
            # 3-panel plot: P vs Depth, T vs Depth, V vs Depth
    ...
    with col2:
        if calc:
            ...
            fig.add_trace(go.Scatter(x=[op_t], y=[op_p], mode='markers',
                                     ...
                                     marker=dict(size=16, color='#009639',
                                                symbol='star')))
...
def batch_upload_page():
    ...
    with col2:
        ...
        if calc:
            # Find pressure/temperature columns
            col_map = {}
